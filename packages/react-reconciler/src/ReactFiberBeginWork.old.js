...
function updateClassComponent(
  current: Fiber | null, // workInPregress会赋值给current树 current在初始化时为null 第一次fiber调和之后，workInPregress会赋值给current树
  workInProgress: Fiber, // 当前正在调和的fiber树  当遍历到一个fiber时 该fiber会指向workInProgress  和current树构成双缓存机制
  Component: any, // class组件
  nextProps: any, // 在一次更新时新的props
  renderLanes: Lanes,
) {
  if (__DEV__) {
  ...
  let shouldUpdate;
  // 组件实例不存在 说明组件没有被挂载 走初始化流程
  if (instance === null) {
    // mount还是update  当前组件fiber节点对应的上一次更新时的fiber节点 即workInProgress.alternate
    if (current !== null) {
    ...
    constructClassInstance(workInProgress, Component, nextProps); // updater里有classComponentUpdater类 方法enqueueSetState等在此阶段被挂载在实例中  new Component()
